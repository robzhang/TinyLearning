...
namespace TinyLearning {
    vector<shared_ptr<Tensor>> Add::Forward(const shared_ptr<Tensor>& x0, const shared_ptr<Tensor>& x1) {
        auto y = x0 + x1;

        return vector<shared_ptr<Tensor>>{y};
    }

    vector<shared_ptr<Variable>> Add::Backward(const vector<shared_ptr<Variable>>& gy) {
        auto gx0 = gy[0];
        auto gx1 = gy[0];
